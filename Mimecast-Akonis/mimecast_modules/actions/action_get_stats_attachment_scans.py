...
class GetStatsAttachmentScansArguments(BaseModel):
...
class MimecastGetStatsAttachmentScans(MimecastAction):
    """Retrieve sandboxing attachment scan statistics from Mimecast."""

    def run(self, arguments: dict[str, Any]) -> dict[str, Any]:
        args = GetStatsAttachmentScansArguments(**arguments)
        ...
        try:
            resp = self.client.get_v2("/threats/v1/stats/attachment-scans", params=params)
            ...
            self.log(message=f"API error retrieving attachment scan stats: {exc}", level="error")
            raise
