...
class GetStatsImpersonationsArguments(BaseModel):
...
class MimecastGetStatsImpersonations(MimecastAction):
    """Retrieve impersonation detection statistics from Mimecast."""

    def run(self, arguments: dict[str, Any]) -> dict[str, Any]:
        args = GetStatsImpersonationsArguments(**arguments)
        ...
        try:
            resp = self.client.get_v2("/threats/v1/stats/impersonations", params=params)
            ...
            self.log(message=f"API error retrieving impersonation stats: {exc}", level="error")
            raise
