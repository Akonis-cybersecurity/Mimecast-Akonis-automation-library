...
class GetThreatsBySenderArguments(BaseModel):
    ...
    email: Optional[str] = Field(None, description="Filter by sender email address")
...
class MimecastGetThreatsBySender(MimecastAction):
    """Retrieve threat statistics grouped by sender from Mimecast."""

    def run(self, arguments: dict[str, Any]) -> dict[str, Any]:
        args = GetThreatsBySenderArguments(**arguments)
        ...
        try:
            resp = self.client.get_v2("/threats/v1/stats/threats-by-sender", params=params)
            ...
            self.log(message=f"API error retrieving threats by sender: {exc}", level="error")
            raise

        payload = resp.json()
        senders = payload.get("data") or payload.get("senders") or []
