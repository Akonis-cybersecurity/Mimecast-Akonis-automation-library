...
SAMPLE_STATS = [{"date": "2026-04-01", "count": 42}]


@pytest.fixture
def action(module):
    a = MimecastGetStatsGatewayDetections(module=module)
    a.log = MagicMock()
    return a
...
class TestGetStatsGatewayDetectionsSuccess:
    def test_returns_stats(self, action):
        _mock_get(action, {"data": SAMPLE_STATS})

        result = action.run({"start": START, "type": "malware"})
    ...
    def test_correct_endpoint(self, action):
        ...
        action.run({"start": START, "type": "spam"})

        assert action.client.get_v2.call_args[0][0] == "/threats/v1/stats/gateway-detections"

    def test_type_param_sent(self, action):
        _mock_get(action, {"data": []})

        action.run({"start": START, "type": "spam"})

        params = action.client.get_v2.call_args[1]["params"]
        assert params["type"] == "spam"

    def test_malware_type_sent(self, action):
        _mock_get(action, {"data": []})

        action.run({"start": START, "type": "malware"})

        params = action.client.get_v2.call_args[1]["params"]
        assert params["type"] == "malware"

    def test_end_param_included_when_provided(self, action):
        _mock_get(action, {"data": []})

        action.run({"start": START, "type": "spam", "end": "2026-04-02T00:00:00+0000"})
    ...
    def test_missing_type_raises(self, action):
        with pytest.raises(Exception):
            action.run({"start": START})

    def test_missing_start_raises(self, action):
        with pytest.raises(Exception):
            action.run({"type": "spam"})


class TestGetStatsGatewayDetectionsErrors:
